...
class SeqRoIHeadsDa(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.embedding_head_class=NormAwareEmbedding()
        # rename the method inherited from parent class
        self.postprocess_proposals = self.postprocess_detections

        self.memory = None # this line was added

        #Add this Function for sampling source and target images
    ...
    def extract_da(self,
        ...
        box_embeddings, __ = self.embedding_head(box_features)
        __, box_cls_scores = self.embedding_head_class(box_features)
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False, is_source=True):
        ...
        if self.training:
            boxes = self.get_boxes(proposal_regs, proposals, image_shapes)
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]
            boxes, matched_idxs, box_pid_labels, box_reg_targets, matched_gt_boxes = self.select_training_samples_gt(boxes, targets) #this line was added
        ...
        # --------------------- Baseline head -------------------- #
        box_features = self.box_roi_pool(features, boxes, image_shapes)
        output_box_features=box_features
        ...
        if self.training:
            ...
            #TODO: add a filtering process in order to learn for IDM
            box_pid_labels_stack=torch.cat(box_pid_labels,dim=0).view(-1)
            #box_pid_labels_stack=box_pid_labels_stack.view(-1)

            # remove negative samples
            inds = torch.nonzero(box_pid_labels_stack > -2).squeeze(1)
            labels, embeddings,feature_maps = (
                    box_pid_labels_stack[inds],
                    box_embeddings[inds],
                    output_box_features[inds]
                )
            result=dict(
                        labels=labels,embeddings=embeddings,feature=feature_maps
                    )
...
#_______________________________________________________________________________
            ...
            for i in range(num_images):
                result.append(
                    dict(
                        boxes=boxes[i], labels=labels[i], scores=scores[i], embeddings=embeddings[i],feature=output_box_features
                    )
                )
