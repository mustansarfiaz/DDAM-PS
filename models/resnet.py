...
class Res5Head(nn.Sequential):
    ...
    def bottleneck_forward(self, bottleneck, x, is_source,is_intermite):
        identity = x
        ...
        if bottleneck.downsample is not None:
            for module in bottleneck.downsample:
                if not isinstance(module, DSBN2d):
                    ...
                    identity = module(identity, is_source,is_intermite)
        out += identity
        out = bottleneck.relu(out)
        return out

    def forward(self, x, is_source=True,is_intermite=False):
        ...
        if is_reid_head:
            ...
            for module in module_seq:
                feat = self.bottleneck_forward(module, feat, is_source,is_intermite)
        else:
            feat = super(Res5Head, self).forward(x)
        x = F.adaptive_max_pool2d(x, 1)
        feat = F.adaptive_max_pool2d(feat, 1)
