...
class BridgeProbLoss(nn.Module):
    ...
    def forward(self, inputs, targets, lam,num_classes=None):
        ...
        soft_targets = (1 - self.epsilon) * targets + self.epsilon / num_classes

        lam = lam.view(-1, 1)
        soft_targets_mixed = lam*targets_s+(1.-lam)*targets_t
        soft_targets_mixed = (1 - self.epsilon) * soft_targets_mixed + self.epsilon / num_classes
