...
class SeqNetDa(nn.Module):
    def __init__(self, cfg):
        ...
        #Add here number of classes for each epoch 
    ...
    def forward(self, images_s, targets_s=None, images_t=None, targets_t=None, query_img_as_gallery=False, is_source=False, epoch=0):
        ...
        results_s, detector_losses_s = self.roi_heads(features_s, proposals_s, images_s.image_sizes, targets_s)
        ...
        if (epoch>=self.target_start_epoch):
            results_t, reid_losses_t = self.roi_heads(features_t, proposals_t, images_t.image_sizes, targets_t, query_img_as_gallery=False, is_source=False)
           ...
           # rename target domain losses
            ...
            losses["loss_box_reg_t"] *= (0.9*self.lw_box_reg)
            losses["loss_box_cls_t"] *= (0.9*self.lw_box_cls)
            losses["loss_box_reid_t"] *= self.lw_box_reid_t

            # Take the features of the second stage for target and source from results of ROIHEAD. 
            box_features_s=results_s["feature"]
            box_features_t=results_t["feature"]
            size=min(box_features_s.size(0),box_features_t.size(0))
            if size > 1:
                concat_features=torch.cat((box_features_s[:size], box_features_t[:size]), dim=0)
                intermite,lam=self.IDM(concat_features)
                intermite_head=self.roi_heads.reid_head(intermite,True,True)
                intermite_embedding,__=self.roi_heads.embedding_head(intermite_head)
                embeddings_s=results_s["embeddings"]
                embeddings_t=results_t["embeddings"]
                concat_prob=torch.cat((embeddings_s[:size],embeddings_t[:size],intermite_embedding),dim=0)
                #[size*3,embedding_size]
                prob=self.roi_heads.memory.forward_prob(concat_prob) #[size*3,classes+clusters+instacses]

                #should have reduce the prob space but due to less information passing it is difcuilt to determine the 
                #source and traget clusters length

                targets = torch.cat((results_s["labels"][:size], results_t["labels"][:size]), dim=0)
                targets = targets.view(-1)

                loss_ce, loss_bridge_prob = self.criterion_ce(prob, targets, lam[:,0].detach(),num_classes=len(self.roi_heads.memory.labels)) #targets is [b*2] and lam is [b,2]
                loss_diverse = self.criterion_diverse(lam)
                loss_bridge_feat = self.criterion_bridge_feat(embeddings_s[:size], embeddings_t[:size], intermite_embedding, lam)

                losses["IDM_CE"]=loss_ce
                losses["IDM_bridge_prob"]=loss_bridge_prob
                losses["IDM_diverse"]=loss_diverse
                losses["IDM_bridge_feat"]=loss_bridge_feat

                        
                losses["IDM_CE"]*=0.5
                losses["IDM_bridge_prob"]*=0.3
                losses["IDM_diverse"]*=1
                losses["IDM_bridge_feat"]*=0.5
        

              #TODO:
              # USe MBCONV from MAXVIT (MBCONV)
              # add IDM in stage 1  
              # polyloss function for class classfication. 

            """
            #Steps:
            1) Take only n samples that are postive (min proposls between source and target)
            1.1) Take the 
            2) Concat the source and target feature maps
            3) calculate the IDM (done)
            4) process Three feature maps and get embedding
            5) calculate the prob of the three embeddings (See how to use the hypridmemeory)
            5.1) reduce the prob to focus on source centroids and target clusters only, remove the instances
            6) pass it through the brigdeProb_loss function(prob,targets,lam)
            7) calcuate the bridgefeat_loss and Divloss (Simple step and doesn't require 5 and 6)
            normlize 
            """
        ...
        if torch.isnan(losses["loss_box_reid_s"]):
            losses.pop("loss_box_reid_s")
            print("REID loss equel Nan")
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        **kwargs
        ...
        self.embedding_head_1 = NormAwareEmbedding()
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        box_embeddings, _ = self.embedding_head(box_features)
        _, box_cls_scores = self.embedding_head_1(box_features)
